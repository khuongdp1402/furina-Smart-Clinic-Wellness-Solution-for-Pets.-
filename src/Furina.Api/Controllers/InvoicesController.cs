...
/// </summary>
public record CreateInvoiceRequest(Guid ClinicId, Guid? VisitId, List<InvoiceLineRequest> Lines, decimal? TotalAmount);
...
public record InvoiceResponse(Guid Id, Guid ClinicId, Guid? VisitId, decimal TotalAmount, DateTimeOffset CreatedAt, List<InvoiceLineResponse> Lines)
{
    public static InvoiceResponse From(Invoice i) => new(
        i.Id, i.ClinicId, i.VisitId, i.TotalAmount, i.CreatedAt, i.Lines.Select(InvoiceLineResponse.From).ToList());
}
